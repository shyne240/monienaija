...
> **V1 Agent reconciliation notice (updated 2026-09-24):** The project owner has approved the Agent ownership reconciliation in [`V1-AGENT-ROADMAP-RECONCILIATION.md`](V1-AGENT-ROADMAP-RECONCILIATION.md) §1A. **No statement in this document is deleted or reworded.** The B5 row in §4 is **narrowed**: B5 retains Agent lifecycle, servicing, operational onboarding workflows and merchant/agent servicing infrastructure, while Agent canonical identity, financial identity/wallet, authentication and transaction authorization are owned by V1 under [`AUTHORITATIVE-V1-PRODUCT-SCOPE.md`](AUTHORITATIVE-V1-PRODUCT-SCOPE.md). The frontend portfolio and maturity gate in §6 are **unchanged**; Agent App sequencing (AGR-02) remains unresolved.
