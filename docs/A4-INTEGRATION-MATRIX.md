...
| **A4T03 — Normalized evidence and immutable snapshot**   | [`A4-SOURCE-EVIDENCE-ADAPTER-CONTRACT.md`](A4-SOURCE-EVIDENCE-ADAPTER-CONTRACT.md), [`A4-NORMALIZED-EVIDENCE-SNAPSHOT.md`](A4-NORMALIZED-EVIDENCE-SNAPSHOT.md), runtime adapters/coordinator/readers in `src/policy/`                                                    | Snapshot scope, source references, freshness states, classifications, canonical hash, no-source-mutation rules, and source-owner read boundaries are implemented.                      | `test/capability-policy-evidence.spec.ts` covers adapter normalization, allowlisting, deterministic assembly, missing/stale/unavailable states, and read-only behavior.                                | Implemented/tested at runtime boundary; live source-data rollout evidence remains separate.                 |
...
| **A4T06 — Versioning and reproducibility**               | [`ADR-0040-Policy-Versioning-and-Reproducibility.md`](ADR/ADR-0040-Policy-Versioning-and-Reproducibility.md), [`A4-POLICY-PERSISTENCE-CONTRACT.md`](A4-POLICY-PERSISTENCE-CONTRACT.md), A4 policy entities/repositories/migration in `src/policy/` and `src/migrations/` | Immutable profile/decision/snapshot linkage, hashes, lineage, expiry, replay, retention, legal-hold metadata, current-effective lookup, and historical reconstruction are implemented. | `test/capability-policy-persistence.spec.ts` covers migration up/down, metadata, hash round-trips, effective-time selection, immutability, currentness, replay, and Operations idempotency adaptation. | Physical artifacts implemented; live database apply/rollback and formal retention review remain separate.   |
| **A4T07 — Deterministic policy evaluation**              | `src/policy/capability-policy.service.ts`, `src/policy/capability-policy.profiles.ts`, `src/policy/capability-policy.types.ts`, physical profile registry                                                                                                                | A2 authorization is consumed separately; effective-time profile selection, immutable snapshots, deterministic decisions, and no financial/source mutation are implemented.             | `test/capability-policy.service.spec.ts` covers replay, changed-payload conflict, failures, read-only evaluation, precedence, limits, and concurrent identical evaluations.                            | Implemented/tested; physical profile selection is available through the A4 policy module.                   |
...
- A4T06 profile/decision/snapshot entities, repositories, retention metadata, immutable-history migration, and rollback SQL are present; live database apply/rollback evidence is not claimed by this implementation task.
- A4T07/A4T09 are wired through TypeORM/Operations adapters in `CapabilityPolicyModule`, while service ports remain available for deterministic tests and alternate approved adapters.
...
- Live database activation, route exposure, and owner approvals remain separate release/governance conditions; they do not remove the committed implementation artifacts.

## 8. Automated validation record

Validation on the A4 implementation completion baseline passed:
...
npx jest test/capability-policy.service.spec.ts \
  ...
  test/capability-policy-recovery.service.spec.ts \
  test/capability-policy-evidence.spec.ts \
  test/capability-policy-persistence.spec.ts --runInBand
