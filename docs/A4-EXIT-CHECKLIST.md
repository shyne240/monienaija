...
| A4T03 | Source adapter and immutable snapshot boundary                                                                 | [`A4-SOURCE-EVIDENCE-ADAPTER-CONTRACT.md`](A4-SOURCE-EVIDENCE-ADAPTER-CONTRACT.md), [`A4-NORMALIZED-EVIDENCE-SNAPSHOT.md`](A4-NORMALIZED-EVIDENCE-SNAPSHOT.md), runtime adapters/coordinator/readers/tests                                                                 | Implemented/tested at runtime boundary; live data rollout evidence separate |
...
| A4T06 | Immutable policy version/decision/snapshot persistence and replay contract                                     | [`ADR-0040-Policy-Versioning-and-Reproducibility.md`](ADR/ADR-0040-Policy-Versioning-and-Reproducibility.md), [`A4-POLICY-PERSISTENCE-CONTRACT.md`](A4-POLICY-PERSISTENCE-CONTRACT.md), policy entities/repositories/migration/tests                                       | Implemented; live database apply/rollback evidence separate                 |
| A4T07 | Deterministic evaluator, decision result, A2 authorization, limits, audit/idempotency ports                    | `src/policy/capability-policy.service.ts`, `src/policy/capability-policy.profiles.ts`, physical profile registry, [`test/capability-policy.service.spec.ts`](../test/capability-policy.service.spec.ts)                                                                    | Implemented/tested with effective-time selection and concurrency coverage   |
...
- [x] Physical A4T06 persistence, immutable snapshot attachments, historical reconstruction, and migration artifacts are implemented; live migration/approval evidence remains separate.
...
| Physical A4T06 persistence              | Blocker for durable production A4       | A4 / Database / Operations                 | Apply migration under controlled backup/rollback and approve retention/replay operations         | Artifact implemented; live activation evidence pending                   |
| Production Operations adapter wiring    | High                                    | Operations / A4                            | Verify shared audit/idempotency/diagnostics/persistence transactionality in deployed environment | Implemented in CapabilityPolicyModule; live operational evidence pending |
| Source adapter production wiring        | High                                    | A4 / source owners                         | Verify approved minimum-field adapters and snapshot attachment controls in deployed environment  | Implemented/wired; live source-data evidence pending                     |
