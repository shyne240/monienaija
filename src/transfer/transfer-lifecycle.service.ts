...
export class TransferLifecycleService {
  ...
  private async createPendingWithinTransaction(
    ...
    const transfer = repository.create({
      ...
      // F-3: representation only. Defaults preserve pre-F-3 behaviour exactly.
      sourceOwnerType: command.sourceOwnerType ?? 'CUSTOMER',
      destinationOwnerType: command.destinationOwnerType ?? 'CUSTOMER',
  ...
  private async recordAudit(
    ...
    await this.auditService.record(manager, {
      ...
      newValues: {
        ...
        sourceOwnerType: transfer.sourceOwnerType,
        destinationOwnerType: transfer.destinationOwnerType,
