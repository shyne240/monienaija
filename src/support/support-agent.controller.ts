...
export class SupportAgentController {
  ...
  async create(
    ...
    const principal = this.requireAgent(req);
  ...
  async list(
    ...
    return this.supportService.listForAgent(principal.agentId!, page, limit);
  }

  @Get('tickets/:id')
  async getOne(@Param('id') id: string, @Req() req: AuthenticatedRequest) {
    const principal = this.requireAgent(req);
    return this.supportService.getForAgent(id, principal.agentId!);
  }
  ...
  async addMessage(
    ...
    const principal = this.requireAgent(req);
    if (dto.isInternal) throw new ForbiddenException('Agent cannot create internal messages');
  ...
  private requireAgent(req: AuthenticatedRequest): AuthorizationPrincipal {
    const principal = req.authorizationPrincipal;
    if (!principal) throw new UnauthorizedException('Authentication required');
    if (principal.type !== 'AGENT') throw new ForbiddenException('Agent access required');
