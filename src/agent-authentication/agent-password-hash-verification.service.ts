...
export type AgentPasswordVerificationFailure = 'MALFORMED_HASH' | 'UNSUPPORTED_ALGORITHM' | 'MISMATCH';

export interface AgentPasswordVerificationResult {
  verified: boolean;
  failure?: AgentPasswordVerificationFailure;
}
...
@Injectable()
export class AgentPasswordHashVerificationService {
  verify(
    password: string,
    algorithm: AgentPasswordHashAlgorithm,
    encodedHash: string,
  ): AgentPasswordVerificationResult {
    ...
    try {
      switch (algorithm) {
        case AgentPasswordHashAlgorithm.PBKDF2:
          return this.verifyPbkdf2(password, encodedHash);
        case AgentPasswordHashAlgorithm.SCRYPT:
          return this.verifyScrypt(password, encodedHash);
        case AgentPasswordHashAlgorithm.ARGON2ID:
        case AgentPasswordHashAlgorithm.BCRYPT:
  ...
  private verifyPbkdf2(password: string, encodedHash: string): AgentPasswordVerificationResult {
  ...
  private verifyScrypt(password: string, encodedHash: string): AgentPasswordVerificationResult {
    const parts = encodedHash.split('$');
  ...
  private compare(actual: Buffer, expected: Buffer): AgentPasswordVerificationResult {
