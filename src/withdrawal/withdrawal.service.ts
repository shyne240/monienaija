...
export class WithdrawalService {
  ...
  /**
   * Customer-scoped read: the withdrawal is only visible to a customer who owns the debited wallet.
   * Cross-customer reads report "not found" so withdrawals cannot be enumerated by id.
   */
  async getWithdrawalForCustomer(
    withdrawalId: string,
    customerId: string,
  ): Promise<WithdrawalView> {
    assertPaymentUuid(withdrawalId, 'withdrawalId');
    const withdrawal = await this.withdrawalRepository.findOne({ where: { id: withdrawalId } });
    if (!withdrawal || !(await this.walletBelongsToCustomer(withdrawal.walletId, customerId))) {
      throw new NotFoundException(`Withdrawal ${withdrawalId} was not found`);
    }
    return this.toView(withdrawal);
  }
  ...
  /**
   * Customer-scoped list: without an explicit wallet this returns only the customer's own
   * withdrawals (never the whole table), and an explicit wallet must belong to the customer.
   */
  async listWithdrawalsForCustomer(
    customerId: string,
    walletId?: string,
  ): Promise<WithdrawalView[]> {
  ...
  private async createWithinTransaction(
    ...
    if (existing) {
      ...
      // key must never return (or reveal) a withdrawal that belongs to that customer.
