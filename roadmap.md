...
> **V1 Agent reconciliation notice (updated 2026-09-24):** The project owner has approved the Agent governance reconciliation recorded in [`docs/V1-AGENT-ROADMAP-RECONCILIATION.md`](docs/V1-AGENT-ROADMAP-RECONCILIATION.md) §1A, alongside the authoritative V1 product scope in [`docs/AUTHORITATIVE-V1-PRODUCT-SCOPE.md`](docs/AUTHORITATIVE-V1-PRODUCT-SCOPE.md). **No statement in this file is deleted, reworded or amended.** The owner has interpreted the human acceptance testing gate below as applying to **Agent client applications (the Agent App) only** — Agent backend identity, wallet, authentication and transaction authorization are not gated by it and proceed under roadmap code **`V1A01`**. "Agent App: NOT STARTED (Requires `B5` backend)" in §2.D stands as written for the Agent App. The fail-closed stop (rule 11) and the unique-task-code requirement (rule 8) remain in force and are not waived.
